...
$rescuersJson = json_encode(array_values($rescuers));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="main-content">
        ...
        <div class="topbar">
            <div class="topbar-left">
                ...
                <span class="page-title">Live Heart Rate Monitor</span>
            </div>
        ...
        <div class="page-content">
            ...
            <div class="stats-grid stats-grid-4 mb-6">
                ...
                <div class="stat-card card-red">
                    <div class="stat-card-header">
                        <div class="stat-icon red"><i class="fa-solid fa-heart-crack"></i></div>
                    </div>
            ...
            <div class="charts-grid">
                ...
                <div class="chart-card">
                    <div class="chart-card-title">
                        <i class="fa-solid fa-chart-pie"></i> Status Distribution
                    </div>
                ...
                <div class="chart-card">
                    <div class="chart-card-title">
                        <i class="fa-solid fa-chart-bar"></i> Heart Rate by Patient
                    </div>
                    <div class="chart-wrapper-tall">
                        <canvas id="bpmBarChart"></canvas>
                    </div>
                </div>
                ...
                <div class="chart-card" style="grid-column:1/-1">
                    <div class="chart-card-title">
                        ...
                        <span style="margin-left:auto;font-size:11px;font-weight:500;color:var(--text-label)">
                            Updates every 3s
                        </span>
                    </div>
                    <div class="chart-wrapper-tall">
                ...
                <div class="section-header">
                    <div>
                        ...
                        <div class="section-subtitle">
                            ...
                            Last: <span id="lastRefreshTime" style="color:var(--text-primary)">—</span>
                        </div>
                    </div>
                    <div class="table-toolbar">
                        <select id="rescuerFilter" class="filter-select"
                                onchange="applyRescuerFilter(this.value)">
                        ...
                        <tbody id="patientTableBody">
                            ...
                            <tr id="row-<?= $p['id'] ?>">
                                ...
                                <td class="td-muted">
                                    ...
                                    <?php else: ?>
                                        <span style="color:var(--text-label);font-size:12px">Unassigned</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <div class="bpm-cell">
                                        <div>
                                            <span class="bpm-value <?= $bpmClass ?>"><?= $hr ?></span>
                                            <span class="bpm-unit"> BPM</span>
                                        </div>
                                        <div class="bpm-bar">
                                            <div class="bpm-bar-fill <?= $fillClass ?>" style="width:<?= $pct ?>%"></div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div style="display:flex;gap:5px">
                                        ...
                                        <button class="btn btn-ghost btn-sm"
                                            onclick="openAlertModal(<?= $p['id'] ?>, '<?= htmlspecialchars($p['name'], ENT_QUOTES) ?>', <?= $p['rescuer_id'] ?? 'null' ?>)"
                                            title="Send alert" style="color:var(--indigo)">
                                            <i class="fa-solid fa-bell"></i>
                                        </button>
                                        <button class="btn btn-ghost btn-sm"
                                            onclick="deletePatient(<?= $p['id'] ?>, '<?= htmlspecialchars($p['name'], ENT_QUOTES) ?>')"
                ...
                <div class="patient-card card-<?= $st ?>" id="card-<?= $p['id'] ?>">
                    ...
                    <div class="patient-card-body">
                        <div class="patient-card-stat">
                            Heart Rate
                            <span class="bpm-value <?= $bpmClass ?>" style="font-size:20px">
                                <?= $hr ?> <small style="font-size:12px;font-weight:400;color:var(--text-muted)">BPM</small>
                            </span>
                        </div>
                        <div class="patient-card-stat">
                            Condition
                        ...
                        <div class="patient-card-stat">
                            Rescuer
                            <span><?= htmlspecialchars($p['rescuer_name'] ?? 'Unassigned') ?></span>
                        </div>
                        <div class="patient-card-stat">
                            Updated
                    ...
                    <div style="display:flex;gap:8px;margin-top:12px">
                        ...
                        <button class="btn btn-ghost btn-sm"
                            onclick="openAlertModal(<?= $p['id'] ?>, '<?= htmlspecialchars($p['name'], ENT_QUOTES) ?>', <?= $p['rescuer_id'] ?? 'null' ?>)"
                            style="flex:1;color:var(--indigo);border-color:var(--indigo-border);justify-content:center">
            ...
            <!-- Add Patient FAB (mobile) -->
            <button onclick="openAddPatientModal()"
                    style="display:none;position:fixed;bottom:80px;right:16px;width:52px;height:52px;border-radius:50%;background:var(--blue);color:#fff;border:none;font-size:22px;cursor:pointer;box-shadow:0 4px 18px rgba(59,130,246,0.4);z-index:900;align-items:center;justify-content:center"
                    id="mobileAddFab">
    ...
    <div class="modal">
        <div class="modal-header">
            <div class="modal-title">
                <i class="fa-solid fa-user-plus" style="color:var(--indigo);margin-right:8px"></i>
                Add New Patient
            </div>
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" onclick="submitAddPatient()">
...
<div class="modal-overlay" id="editPatientModal">
    <div class="modal">
        <div class="modal-header">
            <div class="modal-title">
                <i class="fa-solid fa-pen" style="color:var(--indigo);margin-right:8px"></i>
                Edit Patient
            </div>
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" onclick="submitEditPatient()">
...
<div class="modal-overlay" id="alertModal">
    <div class="modal">
        <div class="modal-header">
            <div class="modal-title">
                <i class="fa-solid fa-bell" style="color:var(--indigo);margin-right:8px"></i>
                Send Alert to Rescuer
            </div>
        ...
        <div class="modal-body">
            ...
            <div class="alert-panel">
                ...
                <div id="alertPatientName"
                     style="font-size:16px;font-weight:700;color:var(--text-primary)"></div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea id="alertMessage" class="form-textarea"
                    placeholder="e.g. Patient is critical — BPM at 145. Please respond immediately."></textarea>
            </div>
            <div class="quick-msg-row">
                <button class="btn btn-ghost btn-sm"
                    onclick="setAlertMsg('Patient is in critical condition. Immediate response required.')">
                    🚨 Critical
                </button>
                <button class="btn btn-ghost btn-sm"
                    onclick="setAlertMsg('Heart rate warning detected. Please check on patient.')">
                    ⚠️ Warning
                </button>
                <button class="btn btn-ghost btn-sm"
                    onclick="setAlertMsg('Please provide status update on this patient.')">
                    📋 Status Check
                </button>
            </div>
...
const statusDoughnut = new Chart(document.getElementById('statusDoughnutChart'), {
    ...
    options: {
        ...
        plugins: {
            legend: {
                position: 'bottom',
                labels: { padding: 16, boxWidth: 10, boxHeight: 10, borderRadius: 5, useBorderRadius: true }
            },
...
const bpmBarChart = new Chart(document.getElementById('bpmBarChart'), {
    ...
    plugins: [{
        ...
        afterDraw(chart) {
            ...
            [{ val: 60, color: 'rgba(16,185,129,.25)' }, { val: 100, color: 'rgba(245,158,11,.25)' }, { val: 120, color: 'rgba(239,68,68,.25)' }]
            .forEach(({ val, color }) => {
...
const trendLineChart = new Chart(document.getElementById('trendLineChart'), {
    ...
    data: {
        labels: trendLabels,
        datasets: [{
            label: 'Avg BPM', data: trendData,
            borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.07)',
            borderWidth: 2.5, pointRadius: 3, pointBackgroundColor: '#6366f1',
    ...
    options: {
        ...
        plugins: {
            legend: { display: false },
            tooltip: { callbacks: { label: ctx => ` Avg: ${ctx.parsed.y.toFixed(1)} BPM` } }
        },
        scales: {
...
function updateBarChart(patients) {
    ...
    bpmBarChart.data.datasets[0].backgroundColor = patients.map(p =>
        p.status === 'critical' ? '#ef4444' : p.status === 'warning' ? '#f59e0b' : '#10b981'
    );
...
async function fetchLiveData() {
    try {
        ...
        data.patients.forEach(p => {
            ...
            const bpmClass = { normal: 'bpm-normal', warning: 'bpm-warning', critical: 'bpm-critical' }[st] || 'bpm-normal';
            ...
            if (row) {
                row.cells[4].innerHTML = `
                    <div class="bpm-cell">
                        ...
                        <div class="bpm-bar"><div class="bpm-bar-fill ${fillCls}" style="width:${pct.toFixed(0)}%"></div></div>
                    </div>`;
            ...
            if (card) {
                ...
                if (bpmEl) {
                    bpmEl.className = `bpm-value ${bpmClass}`;
                    bpmEl.innerHTML = `${hr} <small style="font-size:12px;font-weight:400;color:var(--text-muted)">BPM</small>`;
                }
        ...
        if (activeRescuerFilter && tbody && data.patients.length !== tbody.rows.length) {
            rebuildTable(data.patients);
        }
        ...
        if (criticals.length > 0) {
            ...
            document.getElementById('alertText').textContent =
                criticals.length + ' critical patient(s): ' + criticals.join(', ');
        } else {
...
function applyRescuerFilter(rescuerId) {
    activeRescuerFilter = rescuerId;
    fetchLiveData();
}

function rebuildTable(patients) {
    const tbody = document.getElementById('patientTableBody');
    if (!tbody) return;
    tbody.innerHTML = patients.map(p => {
        ...
        const badgeCls= { normal: 'badge-normal', warning: 'badge-warning', critical: 'badge-critical' }[st] || 'badge-normal';
        const lastUpd = p.last_updated ? new Date(p.last_updated).toLocaleTimeString() : '—';
        const rescName = p.rescuer_name
            ? `<span style="display:flex;align-items:center;gap:5px;font-size:12px"><span style="width:7px;height:7px;border-radius:50%;background:var(--green);display:inline-block"></span>${p.rescuer_name}</span>`
            : `<span style="color:var(--text-label);font-size:12px">Unassigned</span>`;
        return `<tr id="row-${p.id}">
            <td><span style="font-weight:600">${p.name}</span></td>
            <td class="td-muted">${p.age}</td>
            <td class="td-muted">${p.medical_condition || '—'}</td>
            <td class="td-muted">${rescName}</td>
            <td>
                <div class="bpm-cell">
                    <div><span class="bpm-value ${bpmCls}">${hr}</span><span class="bpm-unit"> BPM</span></div>
                    <div class="bpm-bar"><div class="bpm-bar-fill ${fillCls}" style="width:${pct.toFixed(0)}%"></div></div>
                </div>
            </td>
            <td><span class="badge ${badgeCls}">${st.charAt(0).toUpperCase() + st.slice(1)}</span></td>
            <td class="td-muted">${lastUpd}</td>
            <td>
                <div style="display:flex;gap:5px">
                    <button class="btn btn-ghost btn-sm" onclick='openEditModal(${JSON.stringify(p)})' title="Edit"><i class="fa-solid fa-pen"></i></button>
                    <button class="btn btn-ghost btn-sm" onclick="openAlertModal(${p.id},'${p.name.replace(/'/g,"\\'")}',${p.rescuer_id||'null'})" style="color:var(--indigo)" title="Alert"><i class="fa-solid fa-bell"></i></button>
                    <button class="btn btn-ghost btn-sm" onclick="deletePatient(${p.id},'${p.name.replace(/'/g,"\\'")}' )" style="color:var(--red)" title="Delete"><i class="fa-solid fa-trash"></i></button>
                </div>
            </td>
        </tr>`;
    }).join('');
...
async function submitAddPatient() {
    ...
    try {
        const res  = await fetch('../api/patient_crud.php', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name, age: parseInt(age), condition, rescuer_id: rescuerId ? parseInt(rescuerId) : null })
        });
        const data = await res.json();
        if (data.success) {
            closeModal('addPatientModal');
            showToast('Patient Added', 'Patient added successfully.', 'success');
            fetchLiveData();
...
async function submitEditPatient() {
    ...
    try {
        const res  = await fetch('../api/patient_crud.php', {
            ...
            body: JSON.stringify({ id: parseInt(id), name, age: parseInt(age), condition, rescuer_id: rescuerId ? parseInt(rescuerId) : null })
        });
        const data = await res.json();
        if (data.success) {
...
async function submitAlert() {
    ...
    try {
        const res  = await fetch('../api/send_alert.php', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ patient_id: parseInt(patientId), rescuer_id: parseInt(rescuerId), message })
        });
        const data = await res.json();
        if (data.success) {
            closeModal('alertModal');
            showToast('Alert Sent', data.message, 'success');
        } else { showErr(errEl, data.message); }
    } catch(e) { showErr(errEl, 'Network error. Please try again.'); }
}
...
function showToast(title, msg, type = 'success', duration = 4000) {
    if (typeof window.showToast === 'function') { window.showToast(title, msg, type, duration); return; }
    const el = document.createElement('div');
    el.style.cssText = `position:fixed;bottom:80px;right:16px;background:${type==='success'?'#10b981':'#ef4444'};color:#fff;padding:12px 18px;border-radius:10px;z-index:9999;font-size:13px;font-weight:600;box-shadow:0 4px 20px rgba(0,0,0,0.4)`;
