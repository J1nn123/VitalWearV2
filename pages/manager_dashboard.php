 ...
 * Improved: full responsive, high-contrast light theme, no mobile bottom nav.
 */
...
$trendData = $pdo->query("
    SELECT p.name, AVG(h.heart_rate) AS avg_bpm, MAX(h.heart_rate) AS max_bpm,
           MIN(h.heart_rate) AS min_bpm,
           SUM(CASE WHEN h.status='critical' THEN 1 ELSE 0 END) AS critical_count
    FROM patients p JOIN heart_rate_logs h ON h.patient_id = p.id
    WHERE h.timestamp >= NOW() - INTERVAL 1 HOUR
    GROUP BY p.id, p.name ORDER BY critical_count DESC
")->fetchAll();
...
$patientList = $pdo->query("
    SELECT DISTINCT p.id, p.name FROM patients p JOIN heart_rate_logs h ON h.patient_id = p.id
    WHERE h.timestamp >= NOW() - INTERVAL 24 HOUR ORDER BY p.name
")->fetchAll();
...
if (!empty($patientList)) {
    ...
    $histRows = $pdo->query("
        SELECT patient_id, DATE_FORMAT(timestamp,'%H:00') AS hour_label, ROUND(AVG(heart_rate),1) AS avg_bpm
        FROM heart_rate_logs WHERE patient_id IN ($in) AND timestamp >= NOW() - INTERVAL 24 HOUR
        GROUP BY patient_id, DATE_FORMAT(timestamp,'%H:00') ORDER BY timestamp ASC
    ")->fetchAll();
    foreach ($histRows as $row) {
        $patientHistoryMap[$row['patient_id']][] = ['hour' => $row['hour_label'], 'avg_bpm' => (float)$row['avg_bpm']];
    }
}
...
<head>
    ...
    /* ── Stat card icon size for inline SVG ── */
    ...
    /* ── Scrollable table hint on mobile ── */
    ...
    @media (max-width: 640px) {
        ...
        table tbody td:nth-child(3) { display: none; } /* hide condition on small screens */
    }

    /* ── Chart height adjustments ── */
    @media (max-width: 600px) {
        .chart-h-280 { height: 220px !important; }
        .chart-h-260 { height: 200px !important; }
    }
    </style>
</head>
<body>
        ...
        <div class="page-content">
                ...
                <div class="stat-card card-blue">
                    <div class="stat-card-header">
                        <div class="stat-icon blue">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4" stroke-width="2"/></svg>
                        </div>
                    </div>
                ...
                <div class="stat-card card-yellow">
                    <div class="stat-card-header">
                        <div class="stat-icon yellow">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke-width="2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3"/></svg>
                        </div>
                    </div>
                ...
                <div class="stat-card card-purple">
                    <div class="stat-card-header">
                        <div class="stat-icon purple">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                        </div>
                    </div>
            ...
            <div class="section-card">
                <div class="section-header">
                    <div>
                        <div class="section-title">
                            <i class="fa-solid fa-chart-area" style="color:#EF6C52;margin-right:8px;font-size:13px"></i>Hourly Heart Rate Overview
                        </div>
                ...
                <div class="chart-container">
                    <div class="chart-wrapper chart-h-280" style="height:280px">
            ...
            <div class="charts-grid-2">
                <div class="chart-card">
                    <div class="chart-card-title">
                        <i class="fa-solid fa-chart-bar" style="color:#EF6C52"></i> Patient BPM Summary (Last Hour)
                    </div>
                    <?php if (empty($trendData)): ?>
                        <div class="empty-state"><i class="fa-solid fa-chart-bar"></i>No data in the last hour.</div>
                    <?php else: ?>
                        <div class="chart-container">
                            <div class="chart-wrapper chart-h-260" style="height:260px">
                                <canvas id="patientBpmChart"></canvas>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
                <div class="chart-card">
                    <div class="chart-card-title">
                        <i class="fa-solid fa-circle-half-stroke" style="color:#EF6C52"></i> Alert Distribution (All Time)
                    </div>
                    <?php if (($normalCnt + $warnCnt + $critCnt) === 0): ?>
                        <div class="empty-state"><i class="fa-solid fa-chart-pie"></i>No data available.</div>
                    <?php else: ?>
                    <div style="display:flex;align-items:center;justify-content:center;padding:20px;height:260px">
            ...
            <div class="section-card" style="margin-bottom:22px">
                <div class="section-header">
                    <div>
                        <div class="section-title">
                            <i class="fa-solid fa-clock-rotate-left" style="color:#EF6C52;margin-right:8px;font-size:13px"></i>Patient BPM History (Last 24 Hours)
                        </div>
                        <div class="section-subtitle">Hourly average per patient</div>
                    </div>
                </div>
                <?php if (empty($patientList)): ?>
                    <div class="empty-state"><i class="fa-solid fa-chart-line"></i>No BPM history in the last 24 hours.</div>
                <?php else: ?>
                ...
                <div class="chart-container">
                    <div class="chart-wrapper chart-h-280" style="height:280px;padding:0 4px 8px">
            ...
            <div class="section-card">
                <div class="section-header">
                    <div class="section-title">
                        <i class="fa-solid fa-table" style="color:#EF6C52;margin-right:8px;font-size:13px"></i>Patient BPM Detail (Last Hour)
                    </div>
                </div>
                <?php if (empty($trendData)): ?>
                    <div class="empty-state"><i class="fa-solid fa-table"></i>No data in the last hour.</div>
                <?php else: ?>
                ...
                <div class="table-container">
                    <table>
                        ...
                        <?php foreach ($trendData as $t):
                            $bpmClass = $t['avg_bpm']<60||$t['avg_bpm']>120 ? 'bpm-critical' : ($t['avg_bpm']>=100 ? 'bpm-warning' : 'bpm-normal');
                        ?>
                        <tr>
                            <td style="font-weight:700;color:#1A1A2E"><?= htmlspecialchars($t['name']) ?></td>
                            <td><span class="bpm-value <?= $bpmClass ?>"><?= round($t['avg_bpm']) ?></span></td>
                            <td><span style="color:#C45030;font-weight:700"><?= $t['min_bpm'] ?></span></td>
                            <td><span style="color:#B91C1C;font-weight:700"><?= $t['max_bpm'] ?></span></td>
                            <td>
                                <?php if ($t['critical_count'] > 0): ?>
                                    <span class="badge badge-critical"><?= $t['critical_count'] ?> events</span>
                                <?php else: ?>
                                    <span class="badge badge-normal">None</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
            ...
            <div class="section-card">
                <div class="section-header">
                    <div>
                        <div class="section-title">
                            <i class="fa-solid fa-user-injured" style="color:#EF6C52;margin-right:8px;font-size:13px"></i>All Patients
                        </div>
                ...
                <div class="table-container">
                    <table>
                        ...
                        ?>
                        <tr>
                            <td style="font-weight:700;color:#1A1A2E"><?= htmlspecialchars($p['name']) ?></td>
                            ...
                            <td>
                                ...
                                <span style="font-size:11px;color:#9CA3AF;font-weight:500"> BPM</span>
                            ...
                            <td>
                                <div class="row-actions">
                                    <button class="btn btn-ghost btn-sm"
                                        ...
                                        style="color:#4F46E5;border-color:rgba(99,102,241,.25);background:rgba(99,102,241,.06)">
            ...
            <div class="section-card">
                ...
                <div class="table-container">
                    <table>
                        ...
                        ?>
                        <tr>
                            <td style="font-weight:700;color:#1A1A2E"><?= htmlspecialchars($r['full_name']) ?></td>
                            ...
                            <td style="min-width:160px">
                                <div class="load-bar-wrap <?= explode(' ',$lc)[1] ?>">
                                    ...
                                    <span style="font-size:11px;color:#9CA3AF;min-width:32px;font-weight:500"><?= $r['patient_count'] ?>/6</span>
                                </div>
                            </td>
            ...
            <div class="device-stats-grid">
                <div class="device-mini-card">
                    ...
                    <div class="dmc-count" style="color:#065F46"><?= $deviceStats['usable'] ?></div>
                    <div class="dmc-sub">Ready to deploy</div>
                </div>
                <div class="device-mini-card">
                    <div class="dmc-label">In-Use</div>
                    <div class="dmc-count" style="color:#1E40AF"><?= $deviceStats['in-use'] ?></div>
                    <div class="dmc-sub">Currently assigned</div>
                </div>
                <div class="device-mini-card">
                    <div class="dmc-label">Maintenance</div>
                    <div class="dmc-count" style="color:#92400E"><?= $deviceStats['maintenance'] ?></div>
                    <div class="dmc-sub">Under repair</div>
                </div>
                <div class="device-mini-card">
                    <div class="dmc-label">Disposable</div>
                    <div class="dmc-count" style="color:#B91C1C"><?= $deviceStats['disposable'] ?></div>
            ...
            <div class="section-card">
                <div class="section-header">
                    <div>
                        <div class="section-title">
                            <i class="fa-solid fa-microchip" style="color:#EF6C52;margin-right:8px;font-size:13px"></i>All Devices
                        </div>
                ...
                <div class="table-container">
                    <table>
                        ...
                        <?php if (empty($devices)): ?>
                            <tr><td colspan="7" style="text-align:center;color:#9CA3AF;padding:48px;font-size:14px">No devices found.</td></tr>
                        <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td style="font-weight:700;color:#1A1A2E"><?= htmlspecialchars($d['label']) ?></td>
                            <td class="td-muted"><?= htmlspecialchars($d['type']) ?></td>
                            <td style="font-family:monospace;font-size:12px;color:#6B7280;font-weight:500"><?= htmlspecialchars($d['serial_number']??'—') ?></td>
                            <td>
                                ...
                                <?php if ($d['serial_status'] !== 'disposable'): ?>
                                    ...
                                    <?php else: ?>
                                        <form method="POST" style="display:inline">
                                            ...
                                            <input type="hidden" name="device_id" value="<?= $d['id'] ?>">
                                            <button type="submit" class="action-btn action-btn-yellow" onclick="return confirm('Unassign this device?')">
                                                <i class="fa-solid fa-link-slash"></i> Unassign
                                            </button>
                                        </form>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <span style="color:#D1D5DB;font-size:12px;font-weight:500">Decommissioned</span>
                                <?php endif; ?>
                                </div>
                            </td>
                        </tr>
    ...
    <div class="modal-box">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="device_label" class="form-input" placeholder="e.g. Heart Monitor Unit A1" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="serial_number" class="form-input" placeholder="e.g. SN-2024-00123">
            </div>
...
<div class="modal-backdrop" id="assignDeviceModal">
    <div class="modal-box">
        <div class="modal-title">
            <i class="fa-solid fa-link" style="color:#EF6C52;margin-right:8px"></i>Assign Device:
            <span id="assignDeviceName" style="color:#EF6C52"></span>
        </div>
        <form method="POST">
            <input type="hidden" name="action" value="assign_device">
            <input type="hidden" name="device_id" id="assignDeviceId">
            <p style="font-size:13px;color:#6B7280;margin-bottom:16px;font-weight:500">Assign to a patient or a rescuer. Leave the other blank.</p>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="form-label">— or — Assign to Rescuer</label>
            ...
            <div class="modal-body">
                <div class="form-grid">
                    <div class="form-group" style="grid-column:1/-1">
                        ...
                        <input type="text" name="name" class="form-input" placeholder="e.g. Maria Santos" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Age *</label>
                        <input type="number" name="age" min="1" max="120" class="form-input" placeholder="45" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Condition</label>
                        <input type="text" name="condition" class="form-input" placeholder="e.g. Hypertension">
                    </div>
                    <div class="form-group" style="grid-column:1/-1">
                        <label class="form-label">Assigned Rescuer</label>
                        <select name="rescuer_id" class="form-input">
        ...
        <form method="POST" action="?tab=patients">
            ...
            <div class="modal-body">
                <div class="form-grid">
                    <div class="form-group" style="grid-column:1/-1">
                        <label class="form-label">Full Name *</label>
                        <input type="text" name="name" id="editPatientName" class="form-input" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Age *</label>
                        <input type="number" name="age" id="editPatientAge" min="1" max="120" class="form-input" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Condition</label>
                        <input type="text" name="condition" id="editPatientCondition" class="form-input">
                    </div>
                    <div class="form-group" style="grid-column:1/-1">
                        <label class="form-label">Assigned Rescuer</label>
                        <select name="rescuer_id" id="editPatientRescuer" class="form-input">
                            <option value="">None</option>
                            <?php foreach ($rescuers as $r): ?><option value="<?= $r['id'] ?>"><?= htmlspecialchars($r['full_name']) ?></option><?php endforeach; ?>
                        </select>
                    </div>
    ...
    <div class="modal" style="max-width:680px;width:95vw">
        <div class="modal-header">
            <div>
                ...
                <div id="prModalSub" style="font-size:12px;color:#9CA3AF;margin-top:3px;font-weight:500"></div>
            </div>
...
function openPatientReports(patientId, patientName, patientAge, patientCondition) {
    ...
    if (reports.length === 0) {
        list.innerHTML = '<div class="report-empty"><i class="fa-solid fa-inbox" style="font-size:32px;opacity:.25;display:block;margin-bottom:12px"></i>No incident reports found for this patient.</div>';
    } else {
        list.innerHTML = reports.map(r => {
            ...
            return '<div class="report-card">'
                ...
                + '<span><i class="fa-solid fa-user" style="margin-right:4px;color:#EF6C52"></i>' + escHtml(r.rescuer_name) + '</span>'
                + '<span><i class="fa-regular fa-calendar" style="margin-right:4px;color:#9CA3AF"></i>' + dStr + ' at ' + tStr + '</span>'
                + '</div>'
                + (r.description
                    ? '<div class="report-card-desc">' + escHtml(r.description) + '</div>'
                    : '<div style="font-size:12px;color:#9CA3AF;font-style:italic;font-weight:500">No additional description.</div>')
                + '</div>';
        }).join('');
    }
...
Chart.defaults.borderColor  = 'rgba(30,36,80,.07)';
Chart.defaults.font.family  = "'DM Sans', sans-serif";
Chart.defaults.font.weight  = '500';

/* ── Overview: Hourly chart ── */
...
new Chart(document.getElementById('hourlyChart'), {
    ...
    data: {
        ...
        datasets: [{
            ...
            borderColor: '#EF6C52',
            backgroundColor: 'rgba(239,108,82,0.09)',
            borderWidth: 2.5, tension: 0.4, fill: true,
            pointRadius: 5, pointBackgroundColor: '#EF6C52',
    ...
    options: {
        ...
        scales: {
            ...
            y2: {
                ...
                ticks: { font: { size: 12, weight: '500' }, color: '#6B7280' },
                title: { display: true, text: 'Critical Count', color: '#9CA3AF', font: { size: 11 } }
            }
        }
    }
});
...
new Chart(document.getElementById('patientBpmChart'), {
    ...
    data: {
        ...
        datasets: [{
            ...
            backgroundColor: <?= json_encode(array_map(fn($t) =>
                $t['avg_bpm'] < 60 || $t['avg_bpm'] > 120
                    ? 'rgba(239,68,68,0.80)'
                    : ($t['avg_bpm'] >= 100 ? 'rgba(245,158,11,0.80)' : 'rgba(239,108,82,0.80)')
            , $trendData)) ?>,
    ...
    options: {
        ...
        scales: {
            x: {
                grid: { display: false },
                ticks: { font: { size: 11, weight: '600' }, color: '#5A6272', maxRotation: 30 }
            },
            y: {
                grid: { color: 'rgba(30,36,80,.06)' },
                min: 0, max: 160,
...
function updateHistoryChart(patientId) {
    ...
    if (historyChart) {
        ...
        historyChart.data.datasets[0].backgroundColor        = lc.replace(')', ',0.08)').replace('rgb','rgba');
    ...
    historyChart = new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: bpms,
                borderColor: lc,
                backgroundColor: 'rgba(239,108,82,0.07)',
                borderWidth: 2.5, tension: 0.4, fill: true,
        ...
        options: {
            ...
            scales: {
                x: {
                    grid: { color: 'rgba(30,36,80,.06)' },
                    ticks: { font: { size: 11, weight: '500' }, color: '#6B7280' },
                    title: { display: true, text: 'Hour of Day', color: '#9CA3AF', font: { size: 11 } }
                },
                y: {
                    grid: { color: 'rgba(30,36,80,.06)' },
                    min: 40, max: 160,
        ...
        plugins: [{
            ...
            beforeDraw(chart) {
                ...
                if (!y) return;
                [
                    { from: 40, to: 60, color: 'rgba(239,68,68,0.05)' },
                    { from: 60, to: 100, color: 'rgba(239,108,82,0.04)' },
                    { from: 100, to: 120, color: 'rgba(245,158,11,0.05)' },
                    { from: 120, to: 160, color: 'rgba(239,68,68,0.05)' }
                ].forEach(z => {
...
// Init with first patient
(function () {
    const sel = document.getElementById('patientHistorySelect');
    if (sel) updateHistoryChart(sel.value);
})();
<?php endif; ?>
</script>
</body>
