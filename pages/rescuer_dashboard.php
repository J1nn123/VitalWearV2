...
<head>
    ...
    .big-patient-card:hover { background:#FDF8F6; }
    .big-patient-card.critical-card { border-left:4px solid #ef4444; background:rgba(239,68,68,0.04); }
    .big-patient-card.warning-card  { border-left:4px solid #f59e0b; background:rgba(245,158,11,0.03); }
    .big-patient-card.normal-card   { border-left:4px solid #EF6C52; }
    ...
    .alert-item {
        background:#fff;
        border:1px solid rgba(239,108,82,.1);
        border-radius:var(--radius);
        padding:14px 16px;
    }
    .alert-item.unread {
        border-left:4px solid #EF6C52;
    ...
    .alert-item-patient { font-weight:700; font-size:14px; color:#1E2450; }
    .alert-item-time { font-size:11px; color:#9CA3AF; }
    .alert-item-from { font-size:12px; color:#EF6C52; margin-bottom:6px; }
    .alert-item-msg { font-size:13px; color:#374151; line-height:1.5; }
    .unread-dot { width:8px; height:8px; border-radius:50%; background:#EF6C52; display:inline-block; margin-right:5px; }
    ...
    .btn-mark-read {
        ...
        border:1px solid rgba(239,108,82,.2);
        color:#EF6C52;
        border-radius:8px;
        padding:6px 14px;
    ...
    /* Stats grid override for rescuer */
    .stat-card.card-blue  { border-top:3px solid #EF6C52; }
    .stat-card.card-red   { border-top:3px solid #ef4444; }
    .stat-card.card-yellow{ border-top:3px solid #f59e0b; }

    /* Tabs coral override */
    .tabs { border-bottom:2px solid rgba(239,108,82,.15); }
    .tab-btn { color:#6B7280; font-weight:500; border-bottom:2px solid transparent; margin-bottom:-2px; padding:10px 16px; background:none; border-top:none; border-left:none; border-right:none; cursor:pointer; font-size:14px; transition:all .2s; }
    .tab-btn:hover { color:#EF6C52; }
    .tab-btn.active { color:#EF6C52; border-bottom-color:#EF6C52; font-weight:700; }

    /* Critical section header */
    .critical-section-header { background:rgba(239,68,68,.06) !important; }
    .critical-section-title  { color:#ef4444 !important; }

    /* Modal quick-report */
    .modal { background:#fff; }
    .modal-header { border-bottom:1px solid rgba(239,108,82,.12); }
    .modal-footer { border-top:1px solid rgba(239,108,82,.12); }
    .modal-title { color:#1E2450; font-weight:700; }

    /* Unread alerts stat value color */
    ...
    <div class="main-content">
        <div class="topbar">
            ...
            <div class="topbar-right">
                ...
                <div class="live-indicator" style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);color:#ef4444;border-radius:8px;padding:4px 10px;font-size:12px;font-weight:700;display:flex;align-items:center;gap:6px">
        ...
        <div class="page-content">
            ...
            <?php if ($msg): ?>
            <div style="background:<?= $msgType==='success'?'rgba(239,108,82,.08)':'rgba(239,68,68,.08)' ?>;border:1px solid <?= $msgType==='success'?'rgba(239,108,82,.25)':'rgba(239,68,68,.25)' ?>;color:<?= $msgType==='success'?'#EF6C52':'#ef4444' ?>;border-radius:var(--radius-sm);padding:12px 16px;font-size:13px;margin-bottom:16px;display:flex;align-items:center;gap:8px">
            ...
            <div class="section-card" style="margin-bottom:20px">
                <div class="tabs" style="padding:0 20px;display:flex;gap:4px">
                    <button class="tab-btn <?= $tab==='overview'?'active':'' ?>" onclick="location.href='?tab=overview'">
                        <i class="fa-solid fa-gauge-high" style="margin-right:6px;font-size:12px"></i>Dashboard
                    </button>
                    <button class="tab-btn <?= $tab==='patients'?'active':'' ?>" onclick="location.href='?tab=patients'">
                        <i class="fa-solid fa-user-injured" style="margin-right:6px;font-size:12px"></i>My Patients (<?= count($patients) ?>)
                    </button>
                    <button class="tab-btn <?= $tab==='alerts'?'active':'' ?>" onclick="location.href='?tab=alerts'">
                        <i class="fa-solid fa-bell" style="margin-right:6px;font-size:12px"></i>Alerts
                    ...
                    <button class="tab-btn <?= $tab==='report'?'active':'' ?>" onclick="location.href='?tab=report'">
                        <i class="fa-solid fa-file-medical" style="margin-right:6px;font-size:12px"></i>Reports
                    </button>
                </div>
            </div>
                ...
                <div class="big-patient-card critical-card" id="bigcard-<?= $p['id'] ?>">
                    ...
                    <div style="margin-top:12px">
                        <button class="btn btn-danger btn-sm w-full"
                            ...
                            <i class="fa-solid fa-file-circle-exclamation" style="margin-right:5px"></i>Report Incident
                        </button>
                    </div>
                </div>
                <?php endforeach; ?>
            ...
            <!-- ── REPORT TAB ── -->
            <style>
            /* ── Report tab — force light theme on all form fields ── */
            .report-form-card { background:#fff !important; }
            .report-form-card .rfield-label {
                font-size:12px; font-weight:700; color:#374151;
                text-transform:uppercase; letter-spacing:.5px;
                margin-bottom:7px; display:flex; align-items:center; gap:6px;
            }
            .report-form-card .rfield-label i { color:#EF6C52; font-size:11px; }
            .rfield {
                width:100%; box-sizing:border-box;
                background:#F9FAFB;
                border:1.5px solid #424956;
                border-radius:10px;
                padding:11px 14px;
            ...
            .rfield:focus {
                border-color:#EF6C52;
                box-shadow:0 0 0 3px rgba(239,108,82,.12);
                background:#fff;
            }
            select.rfield {
            ...
            .report-history-item:hover { border-color:rgba(239,108,82,.25); box-shadow:0 2px 12px rgba(239,108,82,.08); }
            ...
            .rh-body-meta span { display:flex; align-items:center; gap:4px; }
            ...
            .rh-badge.sev-critical { background:rgba(185,28,28,.1);  color:#b91c1c; border:1px solid rgba(185,28,28,.2); }
            .rh-badge.sev-high     { background:rgba(239,68,68,.1);   color:#ef4444; border:1px solid rgba(239,68,68,.2); }
            .rh-badge.sev-medium   { background:rgba(245,158,11,.1);  color:#d97706; border:1px solid rgba(245,158,11,.2); }
            .rh-badge.sev-low      { background:rgba(239,108,82,.1);  color:#EF6C52; border:1px solid rgba(239,108,82,.22); }
            .rh-time { font-size:11px; color:#9CA3AF; }

            @media(max-width:600px){
                .report-history-item { grid-template-columns:auto 1fr; }
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start">
                ...
                <!-- LEFT: FORM -->
                <div class="section-card report-form-card" style="grid-column:1/-1">
                    <!-- Card header with gradient accent -->
                    <div style="padding:20px 24px 0">
                        <div style="display:flex;align-items:center;gap:12px;padding-bottom:18px;border-bottom:1px solid rgba(239,108,82,.1)">
                            <div style="width:42px;height:42px;border-radius:11px;background:linear-gradient(135deg,rgba(239,108,82,.15),rgba(224,90,58,.08));border:1px solid rgba(239,108,82,.2);display:flex;align-items:center;justify-content:center">
                    ...
                    <div style="padding:20px 24px 24px">
                        <form method="POST" action="?tab=report">
                            ...
                            <div style="margin-bottom:16px">
                                <div class="rfield-label">
                                    <i class="fa-solid fa-circle-exclamation"></i> Severity Level *
                                </div>
            ...
            <div class="section-card" style="margin-top:0">
                <div class="section-header" style="padding:18px 22px 14px">
    ...
    <div class="modal">
        ...
        <form method="POST" action="?tab=report">
            ...
            <div class="modal-body">
                <div style="margin-bottom:14px;padding:12px;background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.2);border-radius:var(--radius-sm)">
...
async function fetchAlerts() {
    try {
        ...
        // Sync sidebar + mobile nav badges
        ...
        // Render inbox only on alerts tab
